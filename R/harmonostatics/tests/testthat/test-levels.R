test_that("we can get the correct interval and level given an integer within, above or below the primary octave", {
  # level = -1
  expect_equal(level_and_interval_for(-13),c(level=-2,interval=11))
  expect_equal(level_and_interval_for(-12),c(level=-1,interval=0))
  expect_equal(level_and_interval_for(-1),c(level=-1,interval=11))
  # level = 0
...
test_that("we can get the correct interval and level given a pair of integers within, above or below the primary octave", {
  ...
  expect_equal(level_and_interval_for(c(-14,27)),c(level=1,interval=1))
})
