...
test_that("potential energy look good", {
  ...
  expect(p, "plot is probably ok")
  p = plot_harmony(list("level:0"=0:12),unlist=TRUE,home=0,columns=c("semitone","potential_energy"),title="Tonic Potential Energy 0",include_names=FALSE)
  expect(p, "plot is probably ok")
  p = plot_harmony(list("level:0"=0:12),unlist=TRUE,home=12,columns=c("semitone","potential_energy"),title="Octave Potential Energy 0",include_names=FALSE)
  expect(p, "plot is probably ok")
